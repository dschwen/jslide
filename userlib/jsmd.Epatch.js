var jsmdE = (function(){
  ...
  function Simulation(w,h, options) {
    ...
    // energy function (crude patch!)
    this.Epair = forceTabulated( energyMorse(1.2) );
    this.Epot = 0.0;
  }
  ...
  function renderAtoms(c) {
    ...
    for( i = 0; i < this.atoms.length; ++i ) {
      ...
      col = Math.floor(50*(this.atoms[i].Epot-44.5));
      col = col>255?255:(col<0?0:col);
      c.fillStyle = 'rgb('+ (1-this.atoms[i].t)*(255-col) + ',' + this.atoms[i].t*(255) + ',' + col + ')';
      x = this.atoms[i].p.x;
      y = this.atoms[i].p.y;
