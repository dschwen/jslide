...
var jslide = (function() {
  ...
  function buildSpotLight(r) {
    ...
    function draw() {
      ...
      if( dirty === null ) {
        ...
        c.clearRect(dirty.x-5,dirty.y,dirty.w+10,dirty.h+10);
        c.fillRect(dirty.x-5,dirty.y,dirty.w+10,dirty.h+10);
      }
    ...
    return {
      ...
      move: function(nx,ny) { 
        ...
        x = Math.floor(nx*scaleFactor-r+o.left);
        y = Math.floor(ny*scaleFactor-r+o.top);
        if( shown ) draw();
      },
      size: function(newr) {
        x = x + r - newr;
  ...
  function scale() {
    var cs = $(slides[current].div),
        ...
        sx = Math.round( (ww-ew*r)/(overview?4/r:2) ),
        sy = Math.round( (wh-eh*r)/(overview?4/r:2) );
